...
def test_backup_publication_and_verification_detect_tampering(tmp_path: Path) -> None:
    ...
    with Project.open(
        ...
        assert migration.schema_version == CURRENT_SCHEMA_VERSION
...
def test_forward_migration_is_backup_first_and_reopens_current_schema(
    ...
    try:
        ...
        for table in (
            ...
            "component_definition_dependencies",
            "component_versions",
    ...
    try:
        assert inspect_database(connection).schema_version == CURRENT_SCHEMA_VERSION

...
and compatible reuse. Plan 15 may analyze/export completed portfolio results but cannot
...
- Point-in-time fitting, preprocessing, selection, refit, release, inference, and training
  audit over exact validation membership
...
- Long/short, position, gross/net, group/factor/benchmark, turnover, liquidity/capacity,
  cash, leverage/margin, and tracking-error constraints
...
6. A causal-fit release is row-relative evidence, not a blanket declassification. A
   ...
   availability are no later than that row's public and, when enabled, project cutoff.
...
11. Ex-ante expected cost and realized execution cost are different typed contracts. A
    fitted optimizer cost is not reported as a realized expense.
...
14. Portfolio construction emits target risky-asset weights plus cash and diagnostic
    intent. It does not emit quantities or orders and cannot mutate accounting state.
...
| `ExpectedCostMaterializationId` | `expected_cost_materialization` | One immutable ex-ante cost estimate occurrence |
...
| `RiskEstimateState` | `computed`, `insufficient_observations`, `asset_missing`, `non_psd`, `invalid_numeric` |
...
| `OptimizationAttemptStatus` | `optimal`, `optimal_inaccurate`, `infeasible`, `unbounded`, `solver_error`, `timeout`, `invalid_result`, `unsupported` |
| `ConstructionStatus` | `completed`, `completed_with_fallback`, `failed` |
| `FallbackKind` | `none`, `retain_current`, `registered_constructor` |
| `TargetWeightState` | `targeted`, `excluded`, `input_missing`, `ineligible`, `fallback_retained` |
...
- definition, version, fit, materialization, construction, attempt, constraint, lineage,
  safety, and event metadata in schema `portfolio`;
...
simulation repositories do not install label adapters. Dynamic relations are reachable
only by typed occurrence handles whose owner, schema, manifest, and project lifecycle are
verified.
...
`InformationClass.LABEL` and `RETROSPECTIVE` roots reject structurally. Opaque/unsafe
causal inputs may execute only through the plan-07 acknowledged research override; their
...
A `TrainingInputCapability` is issued for one exact:
...
- caller operation, expiry within the open project, and one fit request content ID.

It is nonserializable, nontransferable, and does not expose arbitrary SQL or a label
repository. The fit service resolves exact bounded rows internally and records every
...
| `historical_production` | exact samples whose labels were available by each fit cutoff | none | predeclared rolling/expanding fit schedule |
...
The release is valid only for `FitState.COMPLETED`, complete lineage, a safe fit executor,
permitted roles, exact disjointness, and finite validated state. It does not change the
fit's information class. Instead, each prediction row cites the release and proves:
...
`InformationClass.CAUSAL`; a mixed materialization records row states and cannot summarize
as safe causal unless every targeted row passes.
...
participation domain, calibration source/fit when any, aggregation, currency/NAV
normalization, extrapolation/missing behavior, and logical availability. `none` is an
explicit zero-cost model and cannot coexist with other components.
...
constraint-set reference, objective/formulation, solver policy, fallback, and output intent.
It pins exact version/content of referenced definitions at registration or declares typed
version parameters that must resolve before execution identity freezes.
...
Each anchor contains a decision/schedule key, logical cutoff, expanding/rolling/explicit
training selector, fit delay, and the first prediction decision eligible to use the fit.
...
requires randomness, its deterministic seed derives from the fit execution content ID and
the registered seed policy, not worker order.
...
selection manifest names every candidate, fit, score state/value, aggregation, and selected
ordinal before an outer-test capability may open.
...
After fit validation, the service computes the causal-release manifest described in
section 6.3. It rejects release when any fit/scoring role is unauthorized, membership or
root closure is incomplete, a training label was unavailable at the fit cutoff, selection
was not frozen, the final holdout was opened/contaminated too early, or output state is
invalid. Fit completion and release publication are one transaction; a fit never has a
mutable “approved” flag.

### 8.4 Inference and fit selection
...
or z-score to expected return requires registered calibration parameters or a fitted
calibration; a cast/rename is invalid.

A combined forecast consumes exact component forecasts with the same target/event,
horizon, currency/basis, and return convention, or registered conversions. Static weights
are versioned parameters. Learned weights are a fit with the same training/release rules.
Missing-component renormalization is permitted only when declared, requires a minimum
component count, and records the exact participating set per row. Combination precedes and
is independent from portfolio optimization.
...
Probability calibration is a direct registered mapping or fitted calibration under
section 8; clipping arbitrary values to `[0, 1]` does not create a probability.
...
Winsorization uses fixed thresholds or fit state. Learned quantiles follow section 8 and
cannot be estimated on the current full test/holdout cross section unless the transform is
explicitly cross-sectional and uses only contemporaneously available causal values.
...
Historical returns are label-derived estimation inputs accessed only through the risk-fit
capability. The fit retains label ancestry; its point-in-time released covariance/factor
estimate may be consumed by a constructor only when its logical availability satisfies
the decision cutoff under section 6.3.
...
No implementation substitution occurs when the extra is unavailable.
...
fail policy; zero exposure is never an implicit fill.
...
weights `w` at one decision. The canonical trade-weight vector is `delta = w - w0` and
one-way turnover is `0.5 * sum(abs(delta))`. Cost output is a nonnegative fraction of
decision NAV at the optimization horizon. Inputs expressed in money, price, shares,
spread, or volume require exact decision prices, FX, NAV, and quantity/weight conversion
evidence from the current-state capability.
...
- `composite` sums ordered compatible components after unit/NAV conversion.

The exact square-root formula, annualized-to-horizon volatility conversion, ADV window,
price/FX basis, participation cap, and zero/missing-volume behavior are versioned.
Calibration learned from historical fills or returns is a fit and follows section 8. A
user-supplied cost relation has exact keys, availability, units, supported trade domain,
lineage, and safety.
...
row stores proposed delta, input states, expected cost, availability, and reason.
...
fractions of positive decision NAV. Assets follow canonical instrument-ID order. The
target budget is:
...
`0.5 * sum(abs(w_i - w0_i))`; active weight is `w - b`; and tracking error is
...
of causally available volume/ADV or a typed capacity relation. Conversion uses exact NAV,
prices, FX, contract multipliers, and decision availability. It does not enforce order-
...
`exclude_asset` adds exact zero/current-only bounds as declared and records the reason.
...
- optional point-in-time risk, expected-cost, benchmark, factor, group, current-state,
  margin, and borrow capabilities;
...
Long-only equal weight assigns the declared risky net budget equally across eligible
assets. A market-neutral long-short form requires explicit long and short sets and gross
leg budgets; it assigns equal absolute weights within each leg. Empty required legs,
overlap, or incompatible budgets fail. Benchmark-relative equal active weight is a
different constructor version.
...
Quantile long-short uses the exact rank/tie semantics in section 9 and predeclared closed
rank intervals. Boundary ties use the signal's average rank and may make realized leg
counts unequal. Each selected leg then applies equal or absolute-score weighting. This is
a true target portfolio, unlike plan-09 quantile-label diagnostics, and still has no fills
or returns.
...
arbitrary constrained risk parity are unsupported, not approximated.
...
| --- | --- | --- |
| equal weight | signal/eligible set | candidate must already satisfy position, gross/net, cash; otherwise fail/fallback |
| score proportional | allocation score | same common post-check; optional deterministic cap-and-renormalize only as a separately registered algorithm |
| quantile long-short | rank/score | same common post-check with explicit leg budgets |
| inverse volatility | risk diagonal | same common post-check |
...
| mean variance | forecast, PSD covariance, CVXPY | convex hard/soft terms and convex expected cost |
| maximum diversification | PSD covariance/volatility, conic solver | long-only homogeneous terms only |
| benchmark relative | forecast/risk/benchmark, CVXPY | convex active, tracking-error, turnover, cost, and ordinary terms |
...
problem is bounded. Minimum variance omits return/cost unless explicitly registered.
...
states after independently
checking every nonoptional constraint. If current state violates constraints, fallback
...
### 15.1 Current portfolio capability
...
positive NAV, risky weights/quantities, cash by currency, prices/FX/contract multipliers,
...
valuation policy. Negative/zero NAV rejects weight construction. Stale prices follow the
declared valuation policy and remain visible findings.

### 15.2 Target contract

At every targeted decision, a completed construction stores:

- exact decision/cutoff and input occurrence roots;
- every canonical asset with target weight/state/reason and optional current/benchmark/
  active weight;
...
then applies registered capital, risk, exposure, correlation/diversification, and
constraint rules. Parent output is one ordinary absolute/benchmark-relative target and
retains exact child contribution roots.
...
current-state/benchmark/risk/forecast/signal roots, solver policy/capability/problem form,
fallback, target decisions, and intent. It excludes new occurrence IDs, publication times,
physical/staging names, lifecycle event IDs, solver wall-clock duration, logs, and its own
output root.

Equal execution identity must reproduce equal normalized rows/content roots. Equal economic
values reached through a different definition, fit, covariance repair, solver, tolerance,
constraint, or fallback are different executions.
...
- targeted decision/base-key counts equal state/audit counts;
- every computed signal/forecast row has exact input availability and domain evidence;
- every prediction row names zero or one eligible fit and every computed prediction names
  exactly one passing causal release;
...
kind-specific JSON against its schema, and reconciles every ordered child manifest.
...
CREATE TABLE portfolio.forecast_fits (
    ...
    created_at TIMESTAMPTZ NOT NULL,
    CHECK (fit_anchor_at <= fit_cutoff_at),
    ...
    CHECK (
        (fit_state = 'completed'
         ...
         AND causal_release_content_id IS NOT NULL)
        OR
        (fit_state <> 'completed'
...
`validation_plan_id` is mandatory even for `historical_production`: its plan is the exact
time-resolved training-membership authority, though it need not expose an ordinary
evaluation fold. The repository additionally proves the role/capability and holdout
conditions not expressible in checks. For a completed fit, the release verifies
`maximum_input_available_at <= fit_cutoff_at`; the logical delay computes
`fit_available_at` exactly.
...
CREATE TABLE signal_data.signal_values (
    ...
    CHECK ((state = 'computed') = (value IS NOT NULL AND available_at IS NOT NULL))
);
...
CREATE TABLE forecast_data.forecast_values (
    ...
    CHECK (
        ...
        OR (lower_value IS NOT NULL AND upper_value IS NOT NULL)
    )
);
```
...
CREATE TABLE portfolio.risk_model_fits (
    ...
    released_state_content_id VARCHAR,
    implementation_identity_content_id VARCHAR NOT NULL,
    environment_manifest_content_id VARCHAR NOT NULL,
    limits_content_id VARCHAR NOT NULL,
    execution_content_id VARCHAR NOT NULL UNIQUE,
    ...
    CHECK (
        (fit_state = 'completed'
         ...
         AND maximum_input_available_at IS NOT NULL
         AND fit_available_at IS NOT NULL
         AND released_state_content_id IS NOT NULL
         AND causal_release_content_id IS NOT NULL)
        OR
        (fit_state <> 'completed'
         AND fit_available_at IS NULL
         AND released_state_content_id IS NULL
         AND causal_release_content_id IS NULL)
    )
);
...
CREATE TABLE portfolio.risk_materializations (
    ...
    targeted_decision_count BIGINT NOT NULL CHECK (targeted_decision_count >= 0),
...
The risk output schema includes controlled tables keyed by
`(risk_materialization_id, decision_at, risk_model_fit_id)` for estimate headers; ordered
asset rows for volatility/state; canonical lower-triangle covariance rows
...
adjustment norm, horizon/frequency, availability, and causal-release root. Count/root
...
CREATE TABLE portfolio.expected_cost_materializations (
    ...
    semantic_version VARCHAR NOT NULL,
    research_dataset_build_id UUID NOT NULL,
    current_portfolio_state_id UUID NOT NULL,
    input_manifest_content_id VARCHAR NOT NULL,
    parameter_content_id VARCHAR NOT NULL,
    schedule_content_id VARCHAR NOT NULL,
    cutoff_manifest_content_id VARCHAR NOT NULL,
...
CREATE TABLE portfolio.optimization_attempts (
    ...
    status VARCHAR NOT NULL CHECK (
        status IN (
            ...
            'solver_error', 'timeout', 'invalid_result', 'unsupported'
        )
    ),
...
Plan 11 will provide the typed `current_portfolio_state_id`; until then only the
versioned opening-state fixture capability may populate it. A cost materialization is a
parameterized cost surface over proposed deltas, not necessarily one scalar fixed before
optimization; controlled rows store component coefficients/domains and post-target
evaluations under one occurrence.
...
CREATE TABLE portfolio_data.target_weights (
    ...
    state VARCHAR NOT NULL CHECK (
        state IN (
            'targeted', 'excluded', 'input_missing',
            'ineligible', 'fallback_retained'
        )
    ),
    ...
    CHECK ((state IN ('targeted', 'fallback_retained')) = (target_weight IS NOT NULL))
);
...
CREATE TABLE portfolio_data.target_summaries (
    ...
    CHECK (
        (construction_status = 'failed' AND effective_attempt_ordinal IS NULL)
        OR
        (construction_status <> 'failed' AND effective_attempt_ordinal IS NOT NULL)
    )
);
```

Expected-cost component rows are keyed by occurrence/decision/instrument/component and
contain trade domain, coefficients, input state/availability, post-target delta, value,
and reason. Objective-component rows are keyed by result/decision/attempt/component and
store canonical unscaled/scaled values. Multi-strategy contribution rows key each parent
target asset to child intent and contribution; their sum reconciles to the pre-constraint
and final target under the registered attribution rule.

The repository enforces contiguous attempt/term/scope ordinals, exactly one accepted
attempt per completed decision, none per failed decision, effective-attempt agreement,
target/summary/budget/constraint/count/root reconciliation, and one final status over the
complete requested decision interval. A sequence may contain per-decision failures only
when the version declares partial-sequence publication; then top-level status is `failed`
and no downstream simulation adapter is issued until an explicit complete replacement or
policy handles every failed decision.
...
project.portfolio.forecasts.fit(request, capability=training_capability)
project.portfolio.forecasts.materialize(request)
project.portfolio.risk_models.register(definition)
project.portfolio.risk_models.fit(request, capability=training_capability)
...
class ConstructionRequest:
    ...
    current_state: CurrentPortfolioViewRef | None
    constraints: ConstraintSetRef
    expected_cost: ExpectedCostModelRef | None
...
- causal information class, complete root closure, safe or explicitly acknowledged opaque
...
outputs are rejected by default and visibly taint an acknowledged simulation. Structural
label/causal-release failures are always rejected.
...
Dense covariance conversion requires an explicit decision/fit selector and checks
`max_covariance_assets`, entry count, memory, symmetry, asset order, and state. It returns
the ordered asset IDs with the matrix; a matrix alone is never returned. Sparse/factor
views use typed bounded iterators and cannot silently densify.
...
Random estimators/solvers receive a deterministic seed from the execution content manifest
and registered seed namespace. Worker count, chunking, mapping insertion order, locale,
timezone environment, or concurrent request ordering cannot change roots or targets on the
supported platform. A library/solver version, architecture-sensitive algorithm, or changed
...
NAV/FX/price roots are persisted. Negative zero is canonicalized to positive zero only at
...
validate/publish complete bounded diagnostics and its configured fallback; otherwise the
request fails without publication. No path responds by reducing assets, shortening
history, sparsifying covariance, dropping constraints, loosening tolerances, or switching
solvers unless that action is the exact registered fallback.

### 20.3 Custom-code and solver security

Managed/custom implementations reuse plan-08 bounded execution and conformance rules.
Unrestricted Python/SQL remains opaque and cannot access label or fit repositories at
inference. A temporally conforming adapter receives only bounded typed partitions and
cannot return a model object that later opens files/network/SQL during prediction.
...
effective fallback kind, and constraint-evaluation root. It does not include weights,
...
logical fit/value availability remains domain metadata. Exact retry emits no event.
Definition aggregate sequence is the registration sequence; occurrence aggregates use one.
Peer events for a graph publish in deterministic dependency order, with fits before
materializations and all inputs before construction.
...
- `cost_price_missing`, `cost_fx_missing`, `cost_spread_missing`, `cost_volume_missing`,
  `cost_zero_volume`, `cost_domain_exceeded`, `cost_nonconvex`;
- `current_state_missing`, `current_asset_unknown`, `benchmark_missing`,
...
- `symbolic_infeasible`, `solver_missing`, `solver_status`, `solver_timeout`,
...
- A fit logically available exactly at the inclusive public/project cutoff may be used;
  one microsecond later may not.
...
- Negative/zero NAV, missing FX, unavailable current state, or an unknown current position
  prevents weight conversion.
...
- A non-DCP objective/constraint rejects as unsupported; the service does not locally
  linearize it.
- Infeasible, unbounded, timeout, error, or nonfinite candidate statuses remain distinct.
...
- Retain-current fallback fails if current state violates a required target constraint.
...
temporal behavior, fit access if any, convexity/support capabilities, deterministic state,
resource bounds, and acceptance fixtures.
...
- Acknowledged opaque input remains unsafe through signal/forecast/risk/cost/target and is
  rejected by simulator adapters by default; structural violations never override.
...
- Fit state/storage tests cover sample/class/variance insufficiency, singular/nonconverged/
  ...
  versus infrastructure failure.
- Causal-release tests prove the fit remains label-classified, releases are separate and
  immutable, every inequality/root/role/holdout condition gates each prediction, and
  no unmediated fit/label handle reaches decision APIs.
...
- Cost fixtures hand-check fees/spread/linear/square-root/composite values, units/NAV/FX,
  symmetry, zero/missing volume, domain extrapolation, convexity, calibration release, and
  separation from realized cost.
...
- CVXPY/solver tests cover DCP/support preflight, missing extra/solver, exact options,
  deterministic selection, optimal/inaccurate/infeasible/unbounded/error/timeout statuses,
  malformed/nonfinite candidates, residuals, and objective/constraint recomputation.
...
- Target tests reconcile weights/cash/gross/net/turnover/active exposure/risk/cost/objective,
  states/reasons, effective attempt, failed decisions, child contributions, and no order/
  quantity/accounting semantics.
...
- Worker/partition/hash order, locale, supported BLAS thread settings, and insertion order
  preserve fit/value/matrix/target roots; changed code/library/solver environment changes
  execution identity.
...
9. hand the same immutable target contract to plan-12 fixtures without private SQL,
   quantities, orders, fills, accounting, or notebook-only logic.
...
- solver/problem/options/status/attempt/violation evidence, visible fallback, and no silent
  substitution or tolerance relaxation;
...
- multi-strategy resolution to one ordinary parent target before simulation;
...
cutoff. The cumulative review must add this typed adapter to the shared plan-07/08 safety
vocabulary rather than treating it as an opaque exception.
...
conflict is introduced; database-schema and shared-safety additions are recorded for the
post-checkpoint cumulative review.
